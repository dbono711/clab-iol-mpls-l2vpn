"""pyATS job entrypoint for Feature 01 validation.

This job file launches the feature-local AEtest script for the EoMPLS / VPWS
port-mode validation workflow. The testbed is supplied by the pyATS runtime and
...
def main(runtime):
    """Run the Feature 01 pyATS validation script with the active testbed.
    ...
    run(
        testscript=str(tests_dir / "test_vpws_ping.py"),
