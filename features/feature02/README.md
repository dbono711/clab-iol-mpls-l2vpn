# Feature 02 - EoMPLS / VPWS VLAN Mode

## Summary

Feature 02 implements a point-to-point Ethernet service over MPLS using Cisco's L2VPN protocol-based CLI in **VLAN mode**.

This feature introduces:

- **EoMPLS** as the Cisco Ethernet transport mechanism
- **VPWS** as the standards-aligned point-to-point Layer 2 VPN model
- As we are in VLAN mode, this feature supports only tagged frames matching a specific VLAN from the clients (in this case, VLAN 10), untagged frames are not forwarded
...
- [Configuring Ethernet over MPLS in VLAN Mode Using Commands Associated with the L2VPN Protocol-Based CLIs Feature](https://www.cisco.com/c/en/us/td/docs/routers/ios/config/17-x/mpls/b-mpls/m_mp-any-transport-xe.html#task_CAB1D82D953246708848D17715B547FE)
...
```shell
make -C features/feature02 all
make -C features/feature02 validate
make -C features/feature02 clean
```

From inside this directory:

```shell
cd features/feature02
make all
make validate
make clean
```
...
- the resulting configuration reflects a VLAN-mode service setup aligned to the Cisco reference
